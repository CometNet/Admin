...
{{--                         @endif--}}
                     ...
                     @foreach($data as $item)
                         <tr>
                             ...
                             <td>
                                 @if ($item->status == 1)
                                     <a href="{{ route('channel.check', ['id' => $item->getKey(), 'status' => 0]) }}" class="btn btn-danger btn-xs" onclick="return confirm('确定下线吗？')">下线</a>
                                 @elseif($item->status == 0)
                                     <a href="{{ route('channel.check', ['id' => $item->getKey(), 'status' => 1]) }}" class="btn btn-success btn-xs" onclick="return confirm('确定上线吗？')">上线</a>
                                 @endif
                                 <a href="{{ route('channel.edit', ['id' => $item->getKey()]) }}" class="btn btn-primary btn-xs">修改</a>
                                 <button class="btn btn-danger btn-xs"
                                         data-url="{{route('channel.destroy', ['id' => $item->getKey()])}}"
                                         data-toggle="modal"
